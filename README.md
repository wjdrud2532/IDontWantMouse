...
| `macos-keyboard/karabiner.json` | Karabiner-Elements 설정. 넘패드로 마우스 제어 + `Opt+hjkl` 방향키 + `Opt+Space` 레이어(단어·단락 이동) + `Caps Lock` 레이어(포인터 이동·클릭·스크롤) | macOS `~/.config/karabiner/karabiner.json` |
