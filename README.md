...
| `macos-keyboard/karabiner.json` | Karabiner-Elements 설정. 넘패드로 마우스 제어 + `Caps Lock` 레이어(방향키 / +Space 단어·단락 이동) + `Opt` 레이어(마우스 포인터 / +CapsLock 클릭·스크롤) + `₩`(`\`) 키 한영 전환 | macOS `~/.config/karabiner/karabiner.json` |
